...
function normalizeDefinition(
  ...
  if (!ID.test(id))
    throw new TypeError(
      "integration id must be a bounded lowercase identifier"
    );
  if (!VERSION.test(version))
    throw new TypeError("version must use numeric semver major.minor.patch");
  if (input.status !== "available" && input.status !== "disabled")
    throw new TypeError("invalid integration status");
  if (
    ...
    throw new TypeError(
      `capabilities must contain 1-${MAX_CAPABILITIES} items`
    );
  }
  const capabilities = [
    ...new Set(input.capabilities.map(value => value.trim().toLowerCase())),
  ].sort();
  if (capabilities.some(value => !CAPABILITY.test(value)))
...
export class IntegrationCatalog {
  ...
  register(input: IntegrationDefinition): IntegrationDefinition {
    ...
    if (
      !this.records.has(definition.id) &&
      this.records.size >= MAX_INTEGRATIONS
    ) {
  ...
  contract(
    ...
    const required = [
      ...new Set(requiredCapabilities.map(value => value.trim().toLowerCase())),
    ].sort();
    if (required.some(value => !CAPABILITY.test(value)))
      throw new TypeError("invalid required capability");
    return {
      ...
      compatible:
        ...
        required.every(item => record.capabilities.includes(item)),
    };
  }
}
